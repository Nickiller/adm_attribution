...
for(fold in 1:10){
    ...
    train_file = paste('sample_4_train_fold_',fold,'.csv',sep='')
    test_file = paste('sample_4_test_fold_',fold,'.csv',sep='')
    ...
    cv.imp.lr <- train(cv.imp.train$is_CVR~.,data=cv.imp.train,method='glm',family = binomial)
    cat('Generate base model...\n')
    cv.base.lr <- train(cv.base.train$is_CVR~., data=cv.base.train, method='glm', family = binomial)
